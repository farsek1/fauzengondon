...
local function LoadTweensConfig_CreateVisuals()
    -- Сначала создаем палки (RedSticks) из групп
    local groups = {
        {
    ...
    -- Потом загружаем точки для ходьбы из файла или используем стандартные
    local walkPoints = {}
    ...
    if success then
        ...
        if ok and data and data.position then
            for i, p in ipairs(data.position) do
                ...
                walkPoints[#walkPoints + 1] = vec
            end
        end
    end
    
    -- Если файл не загрузился, используем стандартные точки маршрута
    if #walkPoints == 0 then
        walkPoints = {
    ...
    _G.walkPts = walkPoints

    -- Создаем визуальные сферы для точек маршрута
    local folder2 = Instance.new("Folder", workspace)
    folder2.Name = "WalkerSpheres"
    for i, point in ipairs(walkPoints) do
        ...
        part.Position = point
        part.Color = Color3.new(0, 1, 0) -- Зеленые точки для ходьбы
        part.Transparency = 0.25
        part.Name = "WalkerDot_" .. i
    end
end

-- Исправленная функция автоходьбы
local walking = false
...
local function StartWalkingLoop()
    ...
    if not _G.walkPts or #_G.walkPts == 0 then
        warn("No walk points loaded! Load config first.")
        return
    end
    ...
    _G.currentWalkIndex = 1
    
    walkConnection = game:GetService("RunService").Heartbeat:Connect(function()
        if not walking or not _G.walkPts or #_G.walkPts == 0 then
            return
        end
        
        local currentIndex = _G.currentWalkIndex
        local targetPos = _G.walkPts[currentIndex]
        
        if targetPos and character and humanoid and humanoid.Health > 0 then
            ...
            if rootPart then
                ...
                if distance < 8 then
                    _G.currentWalkIndex = currentIndex + 1
                    if _G.currentWalkIndex > #_G.walkPts then
                        _G.currentWalkIndex = 1
                    end
                end
            end
...
-- Создаем вкладки если их нет
if not Tabs and Window then
    Tabs = {
...
-- Добавляем кнопки во вкладку FarmGold
        ...
        while true do
            if not autofarmgoldtoggle.Value then
                ...
                for _, r in pairs(workspace:GetChildren()) do
                    if r:IsA("Model") and (r.Name == "Gold Node" or r.Name:lower():find("gold")) then
